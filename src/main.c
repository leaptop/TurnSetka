...
int l[20];
int p[20];
if (m!=1) printf("Choose winners in each pair of losers bracket! (input 1 if team 1 won, input 2 if team 2 won) \n");
for(i=1;i<(m / 2) + 1;i++){
printf("Winner of pair %d: \n",i);
scanf("%d",&p[i-1]);
if (p[i-1] == 1) l[i-1] = 2;
...
if (m!=1) printf("Choose winners in each pair of winners bracket! (input 1 if team 1 won, input 2 if team 2 won) \n");
for(i=1;i<(m / 2) + 1;i++){
printf("Winner of pair %d: \n",i);
scanf("%d",&n[i-1]);
if (n[i-1] == 1) l[i-1] = 2;
...
if (m == 1){
    ...
    printf("The moment we all came here for! Choose the winner of grand finals between %s(1) and %s(2)! \n",c[0],losers[0]);
    scanf("%d",&n[0]);
    if (n[0]==1) printf("%s is the winner of the tournament!\n",c[0]);
    if (n[0]==2) printf("%s is the winner of the tournament!\n",losers[0]);
}
m=m/2;
if(ls>m && ls!=2) WinnersLL();
//printf("%s, %s \n\n",losers[0],losers[1]);
}
...
int l[20];
int p[20];
if (m!=1) printf("Additional round needed in lower bracket!\n");
printf("Choose winners in each pair! (input 1 if team 1 won, input 2 if team 2 won) \n");
SetPrintL(losers,ls);
for(i=1;i<(ls / 2) + 1;i++){
printf("Winner of pair %d: \n",i);
...
case 2 : if (m!=1) {Winners(); SetPrint(c,m);} if (m==1) printf("%s is the winner of the tournament!\n",c[0]) , ch = 0; break;
}
}
if(q == 1){
    printf("1 - show grid \n");
...
case 2 : WinnersL(); if (m!=0) {SetPrint(c,m); SetPrintL(losers,ls);} if (m==0) ch = 0; break;
}
}
}
}
