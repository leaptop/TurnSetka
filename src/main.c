...
int p[20];
if (m!=1) {
    printf("Choose winners in each pair of losers bracket! (input 1 if team 1 won, input 2 if team 2 won) \n");
}
for(i=1;i<(m / 2) + 1;i++) {
    printf("Winner of pair %d: \n",i);
    scanf("%d",&p[i-1]);
...
int main() {
    ...
    while(ch) {
      if(q <= 0) {
           ...
           case 2 : if (m!=1) {
                    Winners(); SetPrint(c,m);
                } if (m==1) printf("%s is the winner of the tournament!\n",c[0]) , ch = 0; break;
}
}
if(q == 1){
    printf("1 - show grid \n");
